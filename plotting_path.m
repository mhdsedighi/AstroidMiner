...
% 
for i=1:N
    [r,v]=mee2rv(soln(end).grid.state(1:6,i)',params.mu);
...
% Force_mag=zeros(1,N);
for i=1:N
    Force_history_xyz(:,i)=rsw2xyz(U(:,i),R(:,i),V(:,i));
...
plot(T,soln(end).grid.state(4,:),'r.')
ylabel('mee4')
subplot(3,2,5)
hold on
plot(T(1),mee_0(5),'bo')
plot(T(end),mee_f(5),'bo')
plot(T,soln(end).grid.state(5,:),'r.')
ylabel('mee5')
subplot(3,2,6)
hold on
plot(T(1),mee_0(6),'bo')
plot(T,soln(end).grid.state(6,:),'r.')
ylabel('mee6')
